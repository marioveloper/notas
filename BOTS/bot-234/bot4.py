...
def addEvent(update, context):
    ...
    if userisAdmin(chatId, userId, bot) == True:
        if len(args) == 0:
            ...
            bot.sendMessage(
                chat_id = chatId,
                text = f'Por favor ingrese mas informacion para agragar el evento'
            )
        else:
            evento = ' '.join(args)
            eventos = eventos + '\n>>' + evento

            logger.info(f'El usuario ha ingresado un nuevo evento')

            bot.sendMessage(
                chat_id = chatId,
                text = f'{userName} ha anadido un evento correctamente'
            )
    else:
        logger.info(f'{userName} intento agragar un evento')
        bot.sendMessage(
            chat_id = chatId,
            text = f'{userName} no tinees permiso para agragar un evento'
        )
...
def event(update, context):
    ...
    logger.info(f'el usuario {userName} ha solicitaod los eventos')
