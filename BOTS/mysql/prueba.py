        ...
        if message.text.isnumeric():
            try:
                ...
                if id:
                    ...
                    parameters = (datetime.datetime.now(), message.text)
                    cursor.execute(query, parameters)
                    conexion.commit()
                    activar_usuario(message.text)
    ...
    if message.content_type == 'text':
        if message.text.isnumeric():
            try:
                ...
                id = cursor.fetchall()
                if id:
                    query=f'UPDATE investments SET success = 1 WHERE user_id = %s'
...
def registrar_pago(message):
    if message.text == 'Cancelar':
        ...
        if message.content_type == 'photo':
            bot.send_message(CHANNEL_ID, text=f'Prueba Pago\n ID{message.chat.id}')
            time.sleep(3)
